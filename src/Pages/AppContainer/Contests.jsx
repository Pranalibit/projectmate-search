...
const Contests = () => {
    ...
    const filterContests = allContest.filter((contest) => contest.name.toLowerCase().includes(search.toLowerCase()) ||
    ...
    const columns = [
        ...
        {
            name: "Site",
            selector: (row) => (React.createElement("a", { href: row.url, target: "_blank", rel: "noreferrer", className: "flex items-center hover:text-yellow-700 font-medium text-sm " },
                row.site,
                React.createElement(FiExternalLink, { className: "ml-0.5" }))),
            style: {
