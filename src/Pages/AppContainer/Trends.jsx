...
const Trends = () => {
  ...
  useEffect(() => {
    getAllTrend().then((res) => setAllTrend(res));
  }, []);
  console.log(allTrend);

  const filterTrends =  allTrend.filter(
    (trend) =>
      trend.name.toLowerCase().includes(search.toLowerCase()) ||
      trend.site.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div className="pt-16 bg-blue-100 min-h-screen md:px-10 lg:px-16 xl:px-20 space-y-7">
      ...
      <div className="shadow-2xl mx-2 md:mx-0 font-Sora">
        <CustomTable columns={columns} data={filterTrends} />
      </div>
    </div>
  );
};
